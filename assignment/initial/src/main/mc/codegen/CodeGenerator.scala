...
case class Symbol(val name:String,val mtype:Type,val value:Any,var isInit:Boolean)

object CodeGenerator extends Utils {
  val libName = "io"
  def init() = List( Symbol("getInt",MType(List(),IntType),CName(libName),true),
    Symbol("putInt",MType(List(IntType),VoidType),CName(libName),true),
    Symbol("putIntLn",MType(List(IntType),VoidType),CName(libName),true),
    Symbol("getFloat",MType(List(),FloatType),CName(libName),true),
    Symbol("putFloat",MType(List(FloatType),VoidType),CName(libName),true),
    Symbol("putFloatLn",MType(List(FloatType),VoidType),CName(libName),true),
    Symbol("putBool",MType(List(BoolType),VoidType),CName(libName),true),
    Symbol("putBoolLn",MType(List(BoolType),VoidType),CName(libName),true),
    Symbol("putString",MType(List(StringType),VoidType),CName(libName),true),
    Symbol("putStringLn",MType(List(StringType),VoidType),CName(libName),true),
    Symbol("putLn",MType(List(),VoidType),CName(libName),true)
  )
...
class CodeGenVisitor(astTree:AST,env:List[Symbol],dir:File) extends BaseVisitor with Utils {
  ...
  def genMETHOD(consdecl:FuncDecl,o:Any,frame:Frame) = {
    ...
    val symret = Symbol("0_ret",returnType,null,true)
    //start Block
    ...
    emit.printout(emit.emitFUNRETURN(getType(returnType),frame));
    emit.printout(emit.emitENDMETHOD(frame));
    //println("end return: stack = "+frame.getStackSize)
    if(frame.getStackSize!=0)
      println(methodName+"---"+frame.getStackSize+"---:max: "+frame.getMaxOpStackSize)
    //frame.push()
    frame.exitScope();
  }
  ...
  def emitDEFAULT(in:Type,frame:Frame):String = {
    in match {
      ...
      case ArrayType(_,t) => emitDEFAULT(t,frame)
      case ArrayPointerType(t) =>  emitDEFAULT(t,frame)
    }
  }
  ...
  override def visitVarDecl(ast: VarDecl, c: Any): Any = { //added by ledinhtri
    ...
    if(frame != null) {
      ...
      ( emit.emitVAR(index,vname,vtype,frame.getStartLabel,frame.getEndLabel,frame)
        ,
        Symbol(vname,ast.varType,Index(index),if(ast.varType.isInstanceOf[ArrayType]) true else false)
      )
    } //add by ledinhtri
      ...
      SubBody(null,Symbol(vname,ast.varType,CName(className),true)::ctxt.sym)
  ...
  override def visitFuncDecl(ast:FuncDecl,o:Any) = {
    ...
    SubBody(null,Symbol(ast.name.name,MType(listPara, ast.returnType),CName(className),true)::subctxt.sym) //add by ledinhtri - added ast.name.name
  }
  ...
  override def visitBinaryOp(ast: BinaryOp, c: Any): Any = {
    ...
    //println(inre)
    println(ast+" op: stack = "+frame.getStackSize)
    if(op != "=") {
      ...
      op match {
        ...
        case "&&" | "||"  => {
          ...
          val truLabel = frame.getNewLabel()
          val endLabel = frame.getNewLabel()
          val buff = new StringBuffer()

          buff.append(inle)
          if(op == "&&")
            buff.append(emit.emitIFEQ(falseLabel,frame))
          else
            buff.append(emit.emitIFNE(truLabel,frame))

          buff.append(inre)
          buff.append(emit.emitIFEQ(falseLabel,frame))
          buff.append(emit.emitLABEL(truLabel,frame))
          buff.append(emit.emitPUSHICONST(1,frame))
          buff.append(emit.emitGOTO(endLabel,frame))
          buff.append(emit.emitLABEL(falseLabel,frame))
          buff.append(emit.emitPUSHICONST(0,frame))
          buff.append(emit.emitLABEL(endLabel,frame))
          frame.pop()
          (buff.toString, BoolType)
      ...
      if(ast.left.isInstanceOf[ArrayCell]) {
        ...
        //println("assign stack left: "+frame.getStackSize)

        val right =  visit(ast.right,new Access(frame,nenv,false,false)).asInstanceOf[(String,Type)]

        //println("assign stack right: "+frame.getStackSize)
        ...
        if(inlt == inrt) {
          (ctxt.isLeft, ctxt.isFirst) match {
            ...
            {
              //frame.push()
              val store = emit.emitASTORE(getTypeP(left._2),frame)
              //println("assign stack store: "+frame.getStackSize)

              (inle+inre+store,inlt)
            }
            case (false, false) =>
            {
              //frame.push()
              (inle+inre+emit.emitDUPX2(frame)+emit.emitASTORE(getTypeP(left._2),frame),inlt)
            }
        ...
        val right =  visit(ast.right,new Access(frame,nenv,false,false)).asInstanceOf[(String,Type)]
        //println(ast.right+"assign right store: "+frame.getStackSize)
        val left = visit(ast.left,new Access(frame,nenv,true,ctxt.isFirst)).asInstanceOf[(String,Type)]
        //println(left._1+"assign left store: "+frame.getStackSize)
        val inre = right._1
        val inrt = getTypeP(right._2)
        val inle = left._1
        val inlt = getType(left._2)
  ...
  override def visitCallExpr(ast:CallExpr,c:Any) = {
    ...
    println("before CallExpr: stack = "+frame.getStackSize)
    val emits = emitpara+emit.emitINVOKESTATIC(cname+"/"+ast.method.name,ctype,frame)
    println("end CallExpr: stack = "+frame.getStackSize)
    (emits, ctype)
  }
  ...
  override def visitId(ast: Id, c: Any): Any = {
    ...
    if(sym.value.isInstanceOf[Index]) {
      ...
      (ctxt.isLeft, ctxt.isFirst) match {
        ...
        {
          sym.isInit = true
          (emit.emitDUP(frame)+emit.emitWRITEVAR(ast.name,ctype,cindex,frame),sym.mtype)
        }
        case (_,_) => if(!isInit) (emitDEFAULT(sym.mtype,frame),sym.mtype) else (emit.emitREADVAR(ast.name,ctype,cindex,frame),sym.mtype)
      }
    }
    else {
  ...
  override def visitArrayCell(ast: ArrayCell, c: Any): Any = {
    ...
    val inType = arrt match {
      ...
      case MType(_,t) => t match {
        case ArrayPointerType(et) => et
        case et => et
      }
      case _ => arrt
    }
    //emit.print()
    //println(arr+idx+arrt+"\n"+inType)
  ...
  override def visitIf(ast: If, c: Any): Any = {
    ...
    if(getTypeP(condition._2) == BoolType) {
      ...
      //println("if stmt condition: stack = "+frame.getStackSize)
      ...
      //println("if stmt then: stack = "+frame.getStackSize)
      //println("if stmt thenStmt: stack = "+frame.getStackSize)
      ast.elseStmt match {
        case Some(exp) => {
          val labelT = frame.getNewLabel();

          emit.printout(emit.emitGOTO(labelT,frame) + emit.emitLABEL(labelF,frame))
          if(exp.isInstanceOf[Expr])
            emit.printout(visit(exp,new Access(frame,nenv,false,true)).asInstanceOf[(String,Type)]._1)
          else
            visit(exp,new SubBody(frame,nenv))
          //println("if stmt else: stack = "+frame.getStackSize)
    ...
    //println("1 if stmt: stack = "+frame.getStackSize)
    if(frame.getStackSize>1) frame.pop()
    //println("2 if stmt: stack = "+frame.getStackSize)
  }
  ...
  override def visitFor(ast: For, c: Any): Any = {
    ...
    if(condition._2 == BoolType) {
      ...
      //println("Break = "+frame.getBreakLabel)
      //println("endLabel = "+frame.getEndLabel)

      val labelLoop = frame.getStartLabel();
  ...
  override def visitBreak(ast: Break.type, c: Any): Any = {
    val ctxt = c.asInstanceOf[SubBody]
    val frame = ctxt.frame
    //Symbol("0_break",returnType,null,true)::ctxt.sym
  ...
  override def visitReturn(ast: Return, c: Any): Any = {
    ...
    val frame = ctxt.frame
    val nenv = ctxt.sym
    //println("before return: stack = "+frame.getStackSize)
    val x = ast.expr match {
      case Some(exp) => visit(exp, new Access(frame,nenv,false,false)).asInstanceOf[(String,Type)]
      case _ => ("", VoidType)
    }
    //println("after return: stack = "+frame.getStackSize)
    //case class Symbol(val name:String,val mtype:Type,val value:Any)
    val ret = lookup("0_ret",nenv,(m:Symbol)=>m.name) match {
      case Some(Symbol(_,rt,_,_)) => rt
      case _ => VoidType
    }

    //println(ret +"-"+getTypeP(x._2))
    if(ret==FloatType && getTypeP(x._2)==IntType)
      emit.printout(x._1 + emit.emitI2F(frame) + emit.emitGOTO(1,frame))
    else
      emit.printout(x._1 + emit.emitGOTO(1,frame))
    if(ret!=VoidType) frame.pop()
  }
