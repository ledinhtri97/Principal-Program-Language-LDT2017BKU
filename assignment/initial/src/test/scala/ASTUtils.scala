...
object ASTGenerator {
  ...
  def expr(item: Item): Expr = item match {
    ...
    case Term("CallExp", List(Ident(name), ItemList(list)))         => CallExpr(Id(name), list.map(a => expr(a)))
  ...
  def declare(item: Item): Decl = item match {
    case Term("FuncDecl", List(Ident(name), ItemList(args),rtyp,blck))        => FuncDecl(Id(name), args.map(a => declare(a)).toList.asInstanceOf[List[VarDecl]],typ(rtyp),stmt(blck))
